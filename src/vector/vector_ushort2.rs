...
impl Vector for ushort2 {
  ...
  const ZERO: Self = ushort2(0, 0);
  const ONE: Self = ushort2(1, 1);
  const TWO: Self = ushort2(2, 2);
  const THREE: Self = ushort2(3, 3);
  ...
  fn to_char_sat(self) -> char2 {
    return ushort2::to_char(min(self, broadcast(std::i8::MAX as u16)));
  }

  #[inline(always)]
  fn to_uchar_sat(self) -> uchar2 {
    return ushort2::to_uchar(min(self, broadcast(std::u8::MAX as u16)));
  }

  #[inline(always)]
  fn to_short_sat(self) -> short2 {
    return ushort2::to_short(min(self, broadcast(std::i16::MAX as u16)));
  }
  ...
  fn to_int_sat(self) -> int2 {
    return ushort2::to_int(min(self, broadcast(std::i32::MAX as u16)));
  }
  ...
  fn to_long_sat(self) -> long2 {
    return ushort2::to_long(min(self, broadcast(std::i64::MAX as u16)));
  }
