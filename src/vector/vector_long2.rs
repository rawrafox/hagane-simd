...
impl Vector for long2 {
  ...
  const ZERO: Self = long2(0, 0);
  const ONE: Self = long2(1, 1);
  const TWO: Self = long2(2, 2);
  const THREE: Self = long2(3, 3);
  ...
  fn to_char_sat(self) -> char2 {
    return long2::to_char(clamp(self, broadcast(std::i8::MIN as i64), broadcast(std::i8::MAX as i64)));
  }

  #[inline(always)]
  fn to_uchar_sat(self) -> uchar2 {
    return long2::to_uchar(clamp(self, broadcast(std::u8::MIN as i64), broadcast(std::u8::MAX as i64)));
  }

  #[inline(always)]
  fn to_short_sat(self) -> short2 {
    return long2::to_short(clamp(self, broadcast(std::i16::MIN as i64), broadcast(std::i16::MAX as i64)));
  }

  #[inline(always)]
  fn to_ushort_sat(self) -> ushort2 {
    return long2::to_ushort(clamp(self, broadcast(std::u16::MIN as i64), broadcast(std::u16::MAX as i64)));
  }

  #[inline(always)]
  fn to_int_sat(self) -> int2 {
    return long2::to_int(clamp(self, broadcast(std::i32::MIN as i64), broadcast(std::i32::MAX as i64)));
  }

  #[inline(always)]
  fn to_uint_sat(self) -> uint2 {
    return long2::to_uint(clamp(self, broadcast(std::u32::MIN as i64), broadcast(std::u32::MAX as i64)));
  }
  ...
  fn to_ulong_sat(self) -> ulong2 {
    return long2::to_ulong(max(self, Self::ZERO));
  }
}
